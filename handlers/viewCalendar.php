...
$traitements = $pdo->query("SELECT * FROM traitements WHERE id_consultation IN (SELECT id FROM consultations WHERE id_animal IN (SELECT id FROM animaux WHERE decede = FALSE AND id_proprietaire = $user->id))")->fetchAll();
...
foreach ($traitements as $traitement) {
    ...
    $remaining = $traitement->duree_jours + $daysAgo + 1;
